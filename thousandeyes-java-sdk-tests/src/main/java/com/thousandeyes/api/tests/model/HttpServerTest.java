...
public class HttpServerTest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to HttpServerTest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HttpServerTest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in HttpServerTest is not found in the empty JSON string", HttpServerTest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HttpServerTest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HttpServerTest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : HttpServerTest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HttpServerTest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HttpServerTest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HttpServerTest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HttpServerTest.class));

       return (TypeAdapter<T>) new TypeAdapter<HttpServerTest>() {
           @Override
           public void write(JsonWriter out, HttpServerTest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HttpServerTest read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of HttpServerTest given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of HttpServerTest
  * @throws IOException if the JSON string is invalid with respect to HttpServerTest
  */
  public static HttpServerTest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HttpServerTest.class);
  }

 /**
  * Convert an instance of HttpServerTest to an JSON string
  *
  * @return JSON string
  */
