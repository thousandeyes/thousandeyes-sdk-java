...
public class FtpServerTest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to FtpServerTest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FtpServerTest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FtpServerTest is not found in the empty JSON string", FtpServerTest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FtpServerTest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FtpServerTest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : FtpServerTest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!FtpServerTest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'FtpServerTest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<FtpServerTest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(FtpServerTest.class));

       return (TypeAdapter<T>) new TypeAdapter<FtpServerTest>() {
           @Override
           public void write(JsonWriter out, FtpServerTest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public FtpServerTest read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of FtpServerTest given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of FtpServerTest
  * @throws IOException if the JSON string is invalid with respect to FtpServerTest
  */
  public static FtpServerTest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, FtpServerTest.class);
  }

 /**
  * Convert an instance of FtpServerTest to an JSON string
  *
  * @return JSON string
  */
