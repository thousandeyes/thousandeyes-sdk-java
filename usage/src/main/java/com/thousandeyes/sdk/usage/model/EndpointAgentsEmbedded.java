/*
 * Usage API
 * These usage endpoints define the following operations:  * **Usage**: Retrieve usage data for the specified time period (default is one month).          * Users must have the `View organization usage` permission to access this endpoint.     * This operation offers visibility across all account groups within the organization.     * Users with `View organization usage` permission in multiple organizations should query the operation with the `aid` query string parameter (see optional parameters) for each organization.     * The `agentId` field in enterprise agent unit responses may be omitted when not available.  * **Quotas**: Obtain organization and account usage quotas. Additionally, users with the appropriate permissions can create, update, or delete these quotas.          * Users must have the necessary permissions to perform quota-related actions.  Refer to the Usage API operations for detailed usage instructions and optional parameters. 
 *
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package com.thousandeyes.sdk.usage.model;

import java.util.Objects;
import java.util.Map;
import java.util.HashMap;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;


/**
 * EndpointAgentsEmbedded
 */
@JsonPropertyOrder({
  EndpointAgentsEmbedded.JSON_PROPERTY_AID,
  EndpointAgentsEmbedded.JSON_PROPERTY_ACCOUNT_GROUP_NAME,
  EndpointAgentsEmbedded.JSON_PROPERTY_ENDPOINT_AGENTS_EMBEDDED_USED
})
@jakarta.annotation.Generated(value = "com.thousandeyes.api.codegen.ThousandeyesJavaGenerator")
public class EndpointAgentsEmbedded {
  public static final String JSON_PROPERTY_AID = "aid";
  private String aid;

  public static final String JSON_PROPERTY_ACCOUNT_GROUP_NAME = "accountGroupName";
  private String accountGroupName;

  public static final String JSON_PROPERTY_ENDPOINT_AGENTS_EMBEDDED_USED = "endpointAgentsEmbeddedUsed";
  private Long endpointAgentsEmbeddedUsed;

  public EndpointAgentsEmbedded() { 
  }

  public EndpointAgentsEmbedded aid(String aid) {
    this.aid = aid;
    return this;
  }

   /**
   * Unique identifier of the account group that owns the embedded endpoint agents.
   * @return aid
  **/
  @jakarta.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_AID)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public String getAid() {
    return aid;
  }


  @JsonProperty(JSON_PROPERTY_AID)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setAid(String aid) {
    this.aid = aid;
  }


  public EndpointAgentsEmbedded accountGroupName(String accountGroupName) {
    this.accountGroupName = accountGroupName;
    return this;
  }

   /**
   * Name of the account group that owns the embedded endpoint agents.
   * @return accountGroupName
  **/
  @jakarta.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_ACCOUNT_GROUP_NAME)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public String getAccountGroupName() {
    return accountGroupName;
  }


  @JsonProperty(JSON_PROPERTY_ACCOUNT_GROUP_NAME)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setAccountGroupName(String accountGroupName) {
    this.accountGroupName = accountGroupName;
  }


  public EndpointAgentsEmbedded endpointAgentsEmbeddedUsed(Long endpointAgentsEmbeddedUsed) {
    this.endpointAgentsEmbeddedUsed = endpointAgentsEmbeddedUsed;
    return this;
  }

   /**
   * Number of endpoint agents embedded owned by the specific account group in the usage period.
   * @return endpointAgentsEmbeddedUsed
  **/
  @jakarta.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_ENDPOINT_AGENTS_EMBEDDED_USED)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public Long getEndpointAgentsEmbeddedUsed() {
    return endpointAgentsEmbeddedUsed;
  }


  @JsonProperty(JSON_PROPERTY_ENDPOINT_AGENTS_EMBEDDED_USED)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setEndpointAgentsEmbeddedUsed(Long endpointAgentsEmbeddedUsed) {
    this.endpointAgentsEmbeddedUsed = endpointAgentsEmbeddedUsed;
  }


  /**
   * Return true if this EndpointAgentsEmbedded object is equal to o.
   */
  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    EndpointAgentsEmbedded endpointAgentsEmbedded = (EndpointAgentsEmbedded) o;
    return Objects.equals(this.aid, endpointAgentsEmbedded.aid) &&
        Objects.equals(this.accountGroupName, endpointAgentsEmbedded.accountGroupName) &&
        Objects.equals(this.endpointAgentsEmbeddedUsed, endpointAgentsEmbedded.endpointAgentsEmbeddedUsed);
  }

  @Override
  public int hashCode() {
    return Objects.hash(aid, accountGroupName, endpointAgentsEmbeddedUsed);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class EndpointAgentsEmbedded {\n");
    sb.append("    aid: ").append(toIndentedString(aid)).append("\n");
    sb.append("    accountGroupName: ").append(toIndentedString(accountGroupName)).append("\n");
    sb.append("    endpointAgentsEmbeddedUsed: ").append(toIndentedString(endpointAgentsEmbeddedUsed)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

  public static class ModelBuilder {

    private EndpointAgentsEmbedded instance;

    public ModelBuilder() {
      this(new EndpointAgentsEmbedded());
    }

    protected ModelBuilder(EndpointAgentsEmbedded instance) {
      this.instance = instance;
    }

    public EndpointAgentsEmbedded.ModelBuilder aid(String aid) {
      this.instance.setAid(aid);
      return this;
    }
    public EndpointAgentsEmbedded.ModelBuilder accountGroupName(String accountGroupName) {
      this.instance.setAccountGroupName(accountGroupName);
      return this;
    }
    public EndpointAgentsEmbedded.ModelBuilder endpointAgentsEmbeddedUsed(Long endpointAgentsEmbeddedUsed) {
      this.instance.setEndpointAgentsEmbeddedUsed(endpointAgentsEmbeddedUsed);
      return this;
    }

    /**
     * Returns a built EndpointAgentsEmbedded instance.
     *
     * <p>The builder is not reusable.</p>
     */
    public EndpointAgentsEmbedded build() {
      try {
        return this.instance;
      } finally {
        // Ensure that this builder cannot mutate an already-built instance.
        this.instance = null;
      }
    }

    @Override
    public String toString() {
      return getClass() + "=(" + instance + ")";
    }
  }

  /**
   * Create a builder with no initialized fields.
   */
  public static EndpointAgentsEmbedded.ModelBuilder builder() {
    return new EndpointAgentsEmbedded.ModelBuilder();
  }

  /**
   * Create a builder with a shallow copy of this instance.
   */
  public EndpointAgentsEmbedded.ModelBuilder toBuilder() {
    EndpointAgentsEmbedded.ModelBuilder builder = new EndpointAgentsEmbedded.ModelBuilder()
      .aid(getAid())
      .accountGroupName(getAccountGroupName())
      .endpointAgentsEmbeddedUsed(getEndpointAgentsEmbeddedUsed());
    return builder;
  }

}

