...
public class NativeApiClient implements ApiClient {
    ...
    public <T> ApiResponse<T> send(ApiRequest request, Class<T> returnType) throws ApiException {
        ...
        return sendRequestAndProcessResponse(httpRequest, returnType);
    }

    private <T> ApiResponse<T> sendRequestAndProcessResponse(HttpRequest.Builder httpRequest, Class<T> returnType)
            throws ApiException
    {
        try {
            var response = this.getHttpClient().send(httpRequest.build(), HttpResponse.BodyHandlers.ofInputStream());
    ...
    private <T> ApiResponse<T> processResponse(HttpResponse<InputStream> response, Class<T> returnType)
            throws IOException, ApiException
    {
        try(var body = response.body()) {
            ...
            return new ApiResponse<>(
                    response.statusCode(),
                    response.headers().map(),
                    body == null ? null : mapper.readValue(body, returnType)
            );
        }
    }

    private void setMethodAndBody(HttpRequest.Builder httpRequest, ApiRequest request) throws ApiException {
        ...
        if (requestBody instanceof String) {
            ...
            try {
                ...
                httpRequest.method(request.getMethod(), HttpRequest.BodyPublishers.ofByteArray(body));
            } catch (IOException e) {
                throw new ApiException(e);
            }
        }
