...
public class SipServerTest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to SipServerTest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SipServerTest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SipServerTest is not found in the empty JSON string", SipServerTest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SipServerTest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SipServerTest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SipServerTest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SipServerTest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SipServerTest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SipServerTest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SipServerTest.class));

       return (TypeAdapter<T>) new TypeAdapter<SipServerTest>() {
           @Override
           public void write(JsonWriter out, SipServerTest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SipServerTest read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of SipServerTest given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of SipServerTest
  * @throws IOException if the JSON string is invalid with respect to SipServerTest
  */
  public static SipServerTest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SipServerTest.class);
  }

 /**
  * Convert an instance of SipServerTest to an JSON string
  *
  * @return JSON string
  */
