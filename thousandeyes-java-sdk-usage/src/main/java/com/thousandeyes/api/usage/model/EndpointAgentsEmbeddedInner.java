...
public class EndpointAgentsEmbeddedInner {
  ...
  * @throws IOException if the JSON Element is invalid with respect to EndpointAgentsEmbeddedInner
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EndpointAgentsEmbeddedInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in EndpointAgentsEmbeddedInner is not found in the empty JSON string", EndpointAgentsEmbeddedInner.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EndpointAgentsEmbeddedInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EndpointAgentsEmbeddedInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!EndpointAgentsEmbeddedInner.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'EndpointAgentsEmbeddedInner' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<EndpointAgentsEmbeddedInner> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(EndpointAgentsEmbeddedInner.class));

       return (TypeAdapter<T>) new TypeAdapter<EndpointAgentsEmbeddedInner>() {
           @Override
           public void write(JsonWriter out, EndpointAgentsEmbeddedInner value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public EndpointAgentsEmbeddedInner read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of EndpointAgentsEmbeddedInner given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of EndpointAgentsEmbeddedInner
  * @throws IOException if the JSON string is invalid with respect to EndpointAgentsEmbeddedInner
  */
  public static EndpointAgentsEmbeddedInner fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, EndpointAgentsEmbeddedInner.class);
  }

 /**
  * Convert an instance of EndpointAgentsEmbeddedInner to an JSON string
  *
  * @return JSON string
  */
